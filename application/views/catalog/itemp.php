...
<script>
	...
	function timeswitch(elem)
	{
		setTimeout("$('" + elem + "').modal('show')",500);
	}	
	...
	function previewInvoice()
	{
		...
		var valuta = '';
		var total = 0;
		$('#jobs_preview tbody').html('');
			$('.pre_invoice').each(function(){
				name = $(this).find('textarea[name="name[]"]').val();
				days = $(this).find('input[name="date[]"]').val();
				costs = $(this).find('input[name="cost[]"]').val();
				costs = costs.replace(new RegExp(' ','g'),'');
				valuta = $(this).find('select[name="valuta[]"]').val();
				if (name != '') {
					$('#jobs_preview tbody').append('<tr><td>' + name + '</td><td align="center">'+ days +'</td><td align="center">' + costs + ' ' + valuta + '</td></tr>');
					total = Number(total) + Number(costs);
				}	
			});
		$('#pre_itogo').text(total + ' ' + valuta);
	}	
	
	
	
	function previewDogovor()
	{
		var wvhtml = $('.cke_wysiwyg_frame').contents().find('body').html();	
		$('#auth_dolz').html($('.wizard-step-2 input[name=dolz]').val());
		$('#auth_fio').html($('.wizard-step-2 input[name=fio]').val());
		$('#oferta_preview').html(wvhtml);
		//setTimeout("$('#prevDog').click()",500);
	}	
	...
	function switchDogovor(type) {
		...
		//$('.switcher').addClass('');
		$('#of_but_'+type).addClass('btn-inverse');
		$('#of_type_'+type).attr('checked','checked');
	}
...
<script>
	$(document).ready(function() {
		...
		<?endif?>	
		addFile1();
		$('.autonum').autoNumeric('init',{aSep: ' ' , aPad: false});
...
<div class="vertical-box-column" style="min-width:200px;">
		...
		<div class="vertical-box-column" style="padding:0px 15px;min-width:200px">
						<!-- begin panel -->
						<div class="panel panel-primary itempanel" data-sortable-id="table-basic-1" id="profilelist">
							...
							<div class="panel-body centered">
								<div style="text-align:center;overflow:hidden"class="imgr1">
								...
								<p>
											<a href="#modal-docs<?=$exp['id']?>" data-toggle="modal" class="btn btn-sm btn-primary w192 text-left"><i class="fa fa-user"></i> &nbsp;&nbsp;Документы по заявке</a>
								</p>
								

						<div class="modal fade" id="modal-docs<?=$exp['id']?>">
								<div class="modal-dialog"  style="width:400px">
									<div class="modal-content">
										<div class="modal-header">
											<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>

											<h3 >Документы по заявке</h4>
								...
								<?if($imgs->count() > 0):?>
									...
									<?foreach($imgs as $i=>$file) :?>							
										<tr><td class="text-left" style="padding-right:15px;padding-bottom:10px;vertical-align:middle">
...
<div class="modal fade" id="modal-cost1">
	...
	Если Вы не готовы скорректировать предложение, Вы можете самостоятельно подтвердить отказ от заявки или заявка будет удалена автоматически через 3 дня.
								...
								<?else:?>			
										...
										<p><a id="modcost" class="btn btn-sm btn-primary w192 text-left" href="#modal-cost1"  data-toggle="modal"><i class="fa fa-envelope"></i>&nbsp;&nbsp;Ваше предложение</a></p>	
...
<div class="modal fade" id="modal-cp<?=$exp['id']?>">
								<div class="modal-dialog"  style="width:900px">
									<div class="modal-content">
										<div class="modal-header">
											<button type="button" class="close" onclick="$('#modal-cp<?=$exp['id']?>').modal('hide');timeswitch('#modal-cost1')">×</button>
											<h3 ><?//=$exp['shortname']?></h3>
											<div style="clear:both"></div>
										</div>
										<div class="modal-body">												
							...
							<div class="modal fade" id="modal-oferta<?=$exp['id']?>">
								<div class="modal-dialog"  style="width:900px">
									<div class="modal-content">
										<div class="modal-header">
											<button type="button" class="close" onclick="$('#modal-oferta<?=$exp['id']?>').modal('hide');timeswitch('#modal-cost1')">×</button>
											<h3 ><?//=$exp['shortname']?></h3>
											<div style="clear:both"></div>
										</div>
										<div class="modal-body">												
...
<a href="#modal-success" id="a_succ" data-toggle="modal"></a>
							<div class="modal fade" id="modal-success">
								<div class="modal-dialog"  style="width:400px">
									<div class="modal-content">
										...
										<div class="modal-body">
		<div class="alert alert-success m-b-10 m-t-10 text-left">
			<h4><i class="icon-bulb"></i>  Данные Вашего коммерческого предложения успешно сохранены</h4>
		</div>
</div>		
</div>			
								</div>
	...
	<script src="/assets/js/form-wizards-validation.demo1.js"></script>
