<h1 class="page-header">Мои заявки</h1>
		...
		<div  style="padding:0px 20px">
			...
			<div class="row">
			    <!-- begin col-2 -->
			    <div class="col-md-2">
			        <form action="/user/openlots/" method="get" id="sform">
			        ...
			        <div class="hidden-sm hidden-xs">
                        <ul class="nav nav-pills nav-stacked nav-inbox">
							<!--li><a href="javascript:void(0)" onclick="javascript:switchPanels('#st0',$(this).parent())"><table><tr><td>
								...
								<?$folders = array(2=>'text-danger',5=>'text-primary',6=>'text-success',4=>'text-warning')?>
								<?foreach ($stocktype as $i=>$st):?>
									<?//if (count($data[$i]['prods']) > 0) :?>
										...
										<i class="fa fa-fw m-r-5 fa-circle <?=$folders[$i]?>"></i></td><td> <?=$st?>&nbsp;<span class="color:red">(<?=count($data[$i]['prods'])?>)</span></td></tr></table></a></li>
									<?//endif?>	
								<?endforeach?>
                        </ul>
			    ...
			    <div class="col-md-10">
                    <div class="email-btn-row hidden-xs">
                        <a href="/user/addlot" class="btn btn-sm btn-inverse">Создать заявку</a>
						<!--a href="/user/addlot?sale_type=1" class="btn btn-sm btn-inverse"><i class="fa fa-plus m-r-5"></i> Новый вопрос эксперту</a-->
						<a href="#" onclick="$('#delp').submit()" class="btn btn-sm btn-inverse">Удалить</a>
						
								<div class="btn-group">
									<a href="#" class="btn btn-inverse btn-sm " data-toggle="dropdown">Фильтр <i class="fa fa-angle-down m-l-5"></i></a>
									<ul class="dropdown-menu">
									<li <?if(intval($_GET['gr'])==0):?>class="active"<?endif?>><a href="/user/openlots?gr=0">Все</a></li>
									<?foreach ($groups as $i=>$group) :?>						                        					
										<li <?if($i==$_GET['gr']):?>class="active"<?endif?>><a href="/user/openlots?gr=<?=$i?>"><?=$group?></a></li>
										<?$j++?>
									<?endforeach?>
									</ul>
								</div>
						...
						<a href="/user/openlots/" class="btn btn-sm btn-inverse">Все</a>
						<?endif?>
						...
						<?if (count($data[$i]['prods']) > 0) :?>
							<div class="email-content"  id="st<?=$i?>">
									...
									<th style="width:200px">
										Сфера деятельности
												  <!--div class="dropdown">
													<a href="#" class="email-header-link" data-toggle="dropdown"> <i class="fa fa-angle-down m-l-5"></i></a>
													<ul class="dropdown-menu">
								<li <?if(intval($_GET['gr'])==0):?>class="active"<?endif?>><a href="/user/openlots?gr=0">Все</a></li>
							<?foreach ($groups as $i=>$group) :?>						                        					
									<li <?if($i==$_GET['gr']):?>class="active"<?endif?>><a href="/user/openlots?gr=<?=$i?>"><?=$group?></a></li>
							...
							<?endforeach?>
									...
									<td style="text-align:center"><?=date('d.m.Y',$prod['exp'])?>
									
									<a href="javascript:;;" class="btn btn-xs btn-primary">продлить</a>
