...
abstract class Controller_Common extends Controller_Template {
	...
	array(
       ...
       $upmenu = ORM::factory('categorie')->where('enabled','=','1')->where('category_id','=','0')->where('alias','<>','main')->where('nav_type','=','3')->order_by('category_id','ASC')->order_by('sort','ASC')->find_all();
       ...
       $brends = ORM::factory('categorie')->where('enabled','=','1')->where('category_id','=','0')->where('alias','<>','main')->where('nav_type','=','2')->order_by('category_id','ASC')->order_by('sort','ASC')->find_all();
		...
		if(!$this->auth->instance()->logged_in())
		{
			...
			$name = DB::query(Database::SELECT, 'SELECT ss_users.email,ss_useratts.* FROM ss_users LEFT JOIN ss_useratts ON ss_users.id=ss_useratts.user_id WHERE username="'.($this->auth->instance()->get_user()->username).'"')->execute()->as_array();
			$this->login = $name[0];
...
//		print_r($this->banners2);
		$this->banners = ORM::factory('banner')->where('enabled','=',1)->where('type','=',1)->order_by('sort')->find_all();
		$this->banners2 = ORM::factory('banner')->where('enabled','=',1)->where('type','=',2)->order_by('sort')->find_all();

		
        View::set_global('title', $this->gen_info->name);				
        View::set_global('description', $this->gen_info->name);
		//$this->template->cabinet = $this->cabinet();    
        $this->template->vertmenu = $this->make_menu($catalog,"0");
		$this->template->hormenu = $this->make_menu($brends,"0");
		
		$this->template->upmenu = $this->make_menu($upmenu,"0");		
...
/*		if ($data['page']>0)
		...
		//print_r($data);
		$all_prods = DB::query(Database::SELECT, 'SELECT * FROM ss_products WHERE ('.$where.') and enabled=1')->execute()->as_array();
		...
		//echo $where;
		//print_r($data);
		$a['prods'] = DB::query(Database::SELECT, "SELECT ss_products.* FROM ss_products WHERE (".$where.") and enabled=1 ORDER BY article LIMIT ".$items_page." OFFSET ".($data['page']*$items_page))->execute()->as_array();
//		echo count($a['prods']);
		//print_r($a['prods']);
...
function pagination ($total,$per_page,$page,$step,$link) {
	$pages = array();
