
<?if ($state==2):?>
Здравствуйте, <b><?=$user['fio']?> (<?=trim($user['shortname'])?>)</b>!<br />
<br />

К сожалению, Ваше предложение отклонено. <br />
У Вас есть 3 дня, чтобы скорректировать Ваше предложение (по цене, по срокам и т.п.). <br/>Для этого мы рекомендуем еще раз связаться с потенциальным клиентом и прояснить причины отказа.
<br/><br />Если Вы не готовы скорректировать предложение, Вы можете самостоятельно удалить данную заявку из перечня заявок сейчас или заявка будет удалена автоматически через 3 дня.<br />
<br /><a href="http://<?=$_SERVER['HTTP_HOST']?>/<?=$prod->parent_chpu?>/<?=$prod->alias?>.html">УДАЛИТЬ ЗАЯВКУ</a>
<br />
<?endif?>

<br />
<b>Дата:</b> ( <?=date('d.m.Y H:i',$prod->cts)?>)<br /> 
<b><?=$saletypes1[$prod->sale_type]?></b>: <?$cat=ORM::factory('categorie')->where('id','=',$prod->category_id)->find()?>
<?=$cat->name?><br/>
<b>Тема:</b> <?=$prod->title?><br/>
<b><?=$saletypes2[$prod->sale_type]?>:</b> <?=$prod->name?>
<br />

<br />
<br />
С уважением,<br /> НП «ПРОВЭД

<br/><br/>
<img src="http://board.proved-np.org/img/logo.png" />
<br/><br/>
<a href="https://www.facebook.com/provednp">Официальная страница НП ПРОВЭД на Facebook. <br/>
Присоединяйтесь!</a><br/>
 <br/>
<a href="http://www.linkedin.com/company/non-commercial-partnership-for-foreign-trade-proved-?trk=top_nav_home">Follow us! PROVED Partnership Group Page on LinkedIn.<br/>
Learn more about us - PROVED Partnership Official Page on LinkedIn.</a><br/>
 <br/>
<br/>
<p style="font-size:90%">
Данное уведомление сформировано автоматически и не предполагает ответа.<br/>Пожалуйста, не отвечайте на него.<br/>Связаться со службой поддержки НП “ПРОВЭД” можно по электронной почте info@proved-np.org.
<br/><br/>
© 2015. Некоммерческое партнерство профессионалов и участников внешнеэкономической деятельности "ПРОВЭД"
</p>